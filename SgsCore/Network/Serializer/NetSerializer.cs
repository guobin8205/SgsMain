...
namespace SgsCore.Network.Serializer
{
    ...
    public class NetSerializer
    {
        ...
        private class StringSerializer<T> : FastCallSpecific<T, string>
        {
            ...
            public StringSerializer(int size) { _size = size > 0 ? size : 0; }
            public override void Read(T inf, NetDataReader r) { Setter(inf, r.GetString(_size)); }
            public override void Write(T inf, NetDataWriter w) { w.Put(Getter(inf), _size); }
            public override void ReadArray(T inf, NetDataReader r) { SetterArr(inf, r.GetStringArray(_size)); }
            public override void WriteArray(T inf, NetDataWriter w) { w.PutArray(GetterArr(inf), _size); }
        }
        ...
        private ClassInfo<T> RegisterInternal<T>()
        {
            ...
            for (int i = 0; i < props.Length; i++)
            {
                ...
                int strAuthSize = 0;
                if (Attribute.IsDefined(property, typeof(SgsStringMarkerAttribute)))
                {
                    var attr = property.GetCustomAttribute(typeof(SgsStringMarkerAttribute), true);
                    SgsStringMarkerAttribute markerAttr = (SgsStringMarkerAttribute)attr;
                    strAuthSize = markerAttr.Size;
                }
                ...
                if (propertyType.IsEnum)
                {
                    ...
                    serialzer = new StringSerializer<T>(strAuthSize);
